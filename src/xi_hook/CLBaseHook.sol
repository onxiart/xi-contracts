...
// Faithful copy of PancakeSwap Infinity's reference CLBaseHook template.
// Keep style differences (PCS-style shifts, `poolManager`/`vault` immutable casing,
// inline-modifier logic) untouched so future upstream sync stays trivial.
// forge-lint: disable-start(incorrect-shift,screaming-snake-case-immutable,unwrapped-modifier-logic)
...
abstract contract CLBaseHook is ICLHooks {
    ...
    function _hooksRegistrationBitmapFrom(Permissions memory permissions)
        internal
        pure
        returns (uint16)
    {
